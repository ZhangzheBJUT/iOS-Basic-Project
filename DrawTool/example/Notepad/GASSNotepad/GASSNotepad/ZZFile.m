...
/**
 * @description   write object to document file path
 *                if the object has already exit,remove it.
 * @param         object   object to written
 *                path     file path base document
 * @return        BOOL.
 */
+ (BOOL)  WriteObject:(id) object DocumentFilePathForce:(NSString *)fileName
{
    BOOL isExist = [ZZFile FileIsExist:[ZZFile DocumentFilePath:fileName]];
    if (isExist == YES)
    {
        BOOL isSuccess =  [ZZFile removeDocumentFileAtPath:fileName];
        if (isSuccess == NO)
        {
            return NO;
        }
    }
    return   [ZZFile WriteObject:object DocumentFilePath:fileName];
